...
let fontRegular;
let playButton, resetButton, instructionsButton, overlapButton;
...
  instructionsButton.position(20, 140);
}
...
    // Sửa lỗi căn chỉnh nhãn "+17"
    push();
    fill(255, 255, 0);
    textSize(16);
    let xOffset = 0;
    if (this.pos.x < 0) {
      xOffset = 7; // Lệch sang phải 7px cho nhãn bên trái
    } else {
      xOffset = -7; // Lệch sang trái 7px cho nhãn bên phải
    }
    translate(xOffset, 0, 21); // Dịch chuyển nhãn trên trục Z để tránh bị che bởi hạt nhân và dịch chuyển trên trục X
...
      } else if (state === "done" || state === "overlap_spinning") {
        overlapDistance = bondedShellOverlap;
      }

      push();
      if (i === this.shells.length - 1) {
        let currentOffset = (this.pos.x < 0) ? orbitalOffset : -orbitalOffset;
        if (this.pos.x < 0) {
            translate(overlapDistance / 2 + currentOffset, 0, 0);
        } else {
            translate(-overlapDistance / 2 + currentOffset, 0, 0);
        }
      }
...
          } else if (state === "overlap_spinning" && i < 2) {
